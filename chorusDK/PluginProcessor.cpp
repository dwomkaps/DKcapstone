...
#endif
    ), tree(*this, nullptr, "Parameters", createParameters()), highpass1(juce::dsp::IIR::Coefficients<float>::makeHighPass(44100, 20000.0f, 0.1))
#endif
{
    //default values
    //cutoff = 18000;
...
void ChorusDKAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    auto filterContext = juce::dsp::ProcessContextNonReplacing<float>(inputBlock, outputBlock);

    
    mixer.pushDrySamples(inputBlock);

    updateFilter();
    highpass1.process(filterContext);
...
void ChorusDKAudioProcessor::updateFilter()
{
    ...
    *highpass1.state = *juce::dsp::IIR::Coefficients<float>::makeHighPass(lastSampleRate, freq, q);
}
...
juce::AudioProcessorValueTreeState::ParameterLayout ChorusDKAudioProcessor::createParameters()
{
    ...
    params.add(std::make_unique<juce::AudioParameterFloat>("kCutoff", "Cutoff", 20.0, 20000.0, 200.0));
